...
void __dma_callback(void *dma_async_param)
{
...
static bool ion_handle_validate_old(struct ion_client *client, struct ion_handle *handle)
{
	struct rb_node *n = client->handles.rb_node;
...
long sunxi_ion_ioctl(struct ion_client *client, unsigned int cmd, unsigned long arg)
{
	...
	case ION_IOC_SUNXI_PHYS_ADDR:
	{
		...
		if(copy_from_user(&data, (void __user *)arg, sizeof(sunxi_phys_data)))
			return -EFAULT;
		mutex_lock(&client->lock);
		valid = ion_handle_validate_old(client, data.handle);
		mutex_unlock(&client->lock);
		if(!valid)
			return -EINVAL;
...
EXPORT_SYMBOL_GPL(sunxi_ion_ioctl);
#endif
